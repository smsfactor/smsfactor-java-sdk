...
public class ApiRequestor {
	...
	public <T> T request(ApiResource.RequestMethod method, String url, Class<T> clazz, ApiParameters smsFactorParams)
	...
	public <T> InputStream requestDownload(ApiResource.RequestMethod method, String url, ApiParameters smsFactorParams)
			throws SMSFactorException {
		return this.downloadRequest(method, url, smsFactorParams);
	}
	...
	private ApiResponse rawRequest(ApiResource.RequestMethod method, String url, ApiParameters smsFactorParams)
			throws AuthenticationException, ApiConnectionException {
	...
	private InputStream downloadRequest(ApiResource.RequestMethod method, String url, ApiParameters smsFactorParams)
			throws AuthenticationException, ApiConnectionException {
		String apiToken = SMSFactor.apiToken;
		if (apiToken == null || apiToken.trim().isEmpty()) {
			throw new AuthenticationException(
		...
		InputStream in = this.httpClient().requestDownload(method, this.apiBase, url, defaultHeaders, smsFactorParams);
	...
	private <T> T interpretResponse(ApiResponse apiResponse, Class<T> clazz) throws SMSFactorException {
		...
		((SMSFactorResponse) smsFactorResponse).setHttpResponseInformations(apiResponse.statusCode, apiResponse.body,
				apiResponse.headers);
	...
	private void handleErrorResponse(SMSFactorResponse response) throws SMSFactorException {
		...
		if (response.status != null && response.status < 0) {
			...
			// Auth error
			case -1:
				e = new AuthenticationException(response.message);
				break;
			...
			case -6:
				// Data error
			case -7:
				e = new InvalidRequestException(response.message);
				break;

			// Insufficient credits
			case -3:
				e = new InsufficientCreditsException(response.message);
				break;

			// Data error
			case -99:
				e = new UnknownException(response.message);
				break;

			default:
				e = new ApiException(response.message);
				break;
			}
